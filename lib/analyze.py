...
def analyze(data):
    ...
    for batter in batters:
        ...
        if (
            ...
            messages.append(
                "%s: %s hit for the cycle against the %s!"
                % (batter["Team"], batter["Name"], batter["Opponent"])
            )
        if batter["HR"] >= 4:
            messages.append(
                "%s: %s hit %d home runs against the %s!"
    ...
    for pitcher in pitchers:
        ...
        if pitcher["K"] >= 18:
            messages.append(
                "%s: %s struck out %d batters against the %s!"
                % (pitcher["Team"], pitcher["Name"], pitcher["K"], pitcher["Opponent"])
            )
