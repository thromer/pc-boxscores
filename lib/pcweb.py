...
MESSAGE_URL_FMT = 'https://www.pennantchase.com/home/createmessage?email=y&passedlgid=%s'
...
class PcWeb:
    def __init__(self, league_id):
        ...
        login_response = requests.post(LOGIN_URL, data={
            ...
            PASSWORD_FIELD: password}, allow_redirects=False)
        login_response.raise_for_status()
        self.cookies = login_response.cookies
        # print('cookies before', self.cookies)
        home_response = requests.get('https://www.pennantchase.com/lgHome.aspx?lgid=%s' % self.league_id, cookies=self.cookies)
    ...
    def send_to_thromer(self, subject, body):
        post_response = requests.post(MESSAGE_URL_FMT % self.league_id, cookies=self.cookies, data={
    ...
    def league_chat(self, message, trailing_whitespace=0):
        ...
        submit_response = requests.get(SUBMIT_CHAT_URL + 
                                       '?clgid=%s&%s=%s' % (self.league_id, CHAT_MESSAGE_KEY, message + '%C2%A0' * trailing_whitespace), 
                                       allow_redirects=False, cookies=self.cookies, headers=headers)
