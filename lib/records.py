...
BATTING_TUPLES_LIST=[
  ...
  ('E',     False, 'Errors', 'cE', 'history.cE', 'E', None),  # Bad, boring (e.g. shortstop)
...
PITCHING_TUPLES_LIST=[
  ...
  ('HR',       True,  'Homers Allowed', 'cpHR', 'history.cpHR', 'HR', 'HR Allowed'),  # Bad / boring
  ...
  ('QS',       False, 'Quality Starts', 'cQS', 'history.cQS', 'QS', None),  # Boring if you ask me
...
def career_url(league_id, b, p):
  return build_url('https://www.pennantchase.com/lgHistoryCareer.aspx', league_id, b, p)

def single_season_url(league_id, b, p):
  return build_url('https://www.pennantchase.com/lgHistorySingleSeason.aspx', league_id, b, p, args=['tid=0'])
...
def process_table(html_table):
  ...
  for html_row in html_table.find_all('tr'):
    ...
    player_id = re.search(r'(?:^|[^a-z0-9_-])id=([A-Fa-f0-9-]+)(?:$|&)', player_href)[1].upper()
    # print([c.text for c in html_row.find_all('td')])
    team_cell = list(html_row)[team_index]
    team_href = team_cell.find_all('a')[0]['href']
    team_id = re.search(r'(?:^|[^a-z0-9_-])tid=([A-Fa-f0-9-]+)(?:$|&)', team_href)[1].upper()
    result = {'player_id': player_id,
              'team_id': team_id}
    result.update(dict(
      zip(headers, [cell.text for cell in html_row.find_all('td')])))
...
def this_season_thing(db):
  ...
  p.add_argument('-y', '--year', type=int, default=None, required=True)  # TODO infer eventually?
  r = p.parse_args()
  year = r.year
  dry_run = r.dry_run
  
  current_season_b = db.collection(u'current_season_leaders_b')
  current_season_p = db.collection(u'current_season_leaders_p')

  descs_by_header_b = {'Leaders: ' + d['curHeader'] : d for d in BATTING_DESCS if d['curHeader']}
  descs_by_header_p = {'Leaders: ' + d['curHeader'] : d for d in PITCHING_DESCS if d['curHeader']}
  ...
  if dry_run:
    ...
    select = soup.find(lambda tag: tag.has_attr('id') and tag['id'] == 'ContentPlaceHolder1_ddLeague')
    lids = [o['value'] for o in select.find_all('option') if o['value'] != '0']
    for lid in lids:
      url = 'https://www.pennantchase.com/lgLeaders.aspx?lgid=%s&lid=%s' % (LEAGUE_ID, lid)
      r = requests.get(url)
      r.raise_for_status()
  ...
  for content in contents:
    ...
    for html_table in html_tables:
      ...
      for html_row in html_table.find_all('tr'):
        ...
        player_id = re.search(r'(?:^|[^a-z0-9_-])id=([A-Fa-f0-9-]+)(?:$|&)', href)[1].upper()
...
def another_thing(db):
  ...
  off_season_b = db.collection(u'off_season_single_season_leaders_b')
  off_season_p = db.collection(u'off_season_single_season_leaders_p')
    ...
    if bsk or psk:
      ...
      if bsk:
        ...
        for v in batting:
          ...
          value = {'value': number(v[b['sHeader']]),
                   'Year': number(v['Year'])}
          value.update({
            k: v[k] for k in ['player_id', 'Name', 'Team', 'team_id']})
          values.append(value)
        values.sort(key=lambda x: (x['value'],x['Year'],x['player_id']), reverse=True)
        if len(values) != 15:
      ...
      if psk:
        ...
        for v in pitching:
          ...
          value = {'value': number(v[p['sHeader']]),
                   'Year': number(v['Year'])}
          value.update({
            k: v[k] for k in ['player_id', 'Name', 'Team', 'team_id']})
          values.append(value)
...
def merge_season_thing(db):
  ...
  current_season_b = db.collection(u'current_season_leaders_b')
  current_season_p = db.collection(u'current_season_leaders_p')
  live_season_b = db.collection(u'live_single_season_leaders_b')
  live_season_p = db.collection(u'live_single_season_leaders_p')
  ...
  for desc in [d for d in BATTING_DESCS if d['inclSingle']]:
    ...
    off.sort(key=lambda x: (x['value'],x['Year'],x['player_id']), reverse=True)  # TODO remove this is because i backfilled with wrong order at first also sorting doesn't really matter
    ...
    if merged != old_live:
      ...
      for k, v in merged_ranked.items():
        o = old_live_ranked[k] if k in old_live_ranked else {'rank': 'N/A', 'tie': 'N/A', 'value': 'N/A'}
        if (not k in old_live_ranked) or (v['rank'] < o['rank']):
          message = '%s: %s %s moved up from rank %s (tie %s) to rank %s (tie %s) val went from %s to %s' % (
            key, v['Name'], v['Year'], o['rank'], o['tie'], v['rank'], v['tie'], o['value'], v['value'])
          print(message)
