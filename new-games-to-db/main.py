...
def get_year_from_db_maybe_update(db: firestore.Client, day: int, dry_run: bool) -> int:
    ...
    if day >= 8:
        ...
        print(f"{day=}: using current_year from firestore {current=}")
        return current["year"]
    ...
    if current and pc_year != current["year"] + 1:
        raise Exception(
            f"{day=}, {current=}: expected pc_year == db_year + 1 but {pc_year=} db_year={current['year']}"
        )
    new_current = {"year": pc_year, "timestamp": now}
    if not dry_run:
        print(f"Setting current_year in database {new_current=}")
        ref.set(new_current)
    else:
        print(f"Dry run, would have set current_year in database {new_current=}")
    return pc_year
    ...
    while day >= 1 and considered < limit:
        ...
        for score_table in score_tables:
            ...
            for line_raw in away_home_raw:
                ...
                team_id = re.match(
                    r".*tid=([^&]+)", line_elts[0].find_all("a")[0]["href"]
                )[1]
                away_home_ids.append(team_id)
                away_home_runs.append(int(line_elts[11].text))
                box_score_url = (
                    "https://www.pennantchase.com/" + rows[-1].find_all("a")[0]["href"]
                )
